  ...
  try {
    ...
    if(category === 'REWARDS') {
      ...
      if(contents == 'WELCOME' || contents == 'PHR') {
        ...
        const gemHistory = await prisma.gemTransactions({where: {rewardType: contents, receiverUserRowId: res.locals.user.id, status: true, createTime_gte: new Date(`${today.getFullYear()}-${today.getMonth() +1 }-01`)}, orderBy: 'createTime_DESC'});
        if(gemHistory.length > 0) {
